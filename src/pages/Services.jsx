export default function Services() {
  // Datos de servicios con textos enfocados en lo que buscan las empresas
  const services = [
    {
      id: 1,
      ...
      icon: (
        <svg className="w-8 h-8 text-slate-400 group-hover:text-green-500 transition-colors" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
  ...
  return (
    <section id="servicios" className="p-6 md:p-10 bg-slate-950 relative">
      ...
      <div className="text-center mb-12 max-w-4xl mx-auto">
        <p className="text-green-500 font-bold uppercase tracking-widest text-lg mb-4">
          • ¿Qué ofrezco?
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {services.map((service) => (
          <div
            key={service.id}
            className="group p-8 rounded-2xl border border-slate-800 bg-slate-900/50 hover:bg-slate-900 hover:border-green-500/50 transition-all duration-300 hover:-translate-y-1"
          >
            {/* Icono */}
            <div className="mb-6">
              {service.icon}
